...
function updateComment(url, request) {
  ...
  if (!id || !requestComment) {
    ...
    const newBody = request.body.comment.body;
    if (!newBody) {
      response.status = 400;
    } else {
      database.comments[id].body = newBody;
      response.body = {};
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    // Made more sense to delete comment rather than store as null
    delete database.comments[id]// = null;

    //Find the comments article
    const article = savedComment.articleId;
    //Find the comments position in the articles comment array
    const articleComPos = database.articles[article].commentIds.indexOf(id);
    //Remove the element at the previously found position
    database.articles[article].commentIds.splice(articleComPos,1);

    // As above but for user instead of article
    const user = savedComment.username;
    const userComPos = database.users[user].commentIds.indexOf(id);
    database.users[user].commentIds.splice(userComPos,1);
...
function upvoteComment(url, request) {
  ...
  if (!comment || !user) {
    response.status = 400;
  } else if (!comment.upvotedBy.includes(username)) {
    database.comments[id].upvotedBy.push(username);
    if (comment.downvotedBy.includes(username)) {
      const pos = comment.downvotedBy.indexOf(username);
      database.comments[id].downvotedBy.splice(pos,1);
    }
    response.status = 200;
    response.body = {};
    response.body.comment = database.comments[id];
...
function downvoteComment(url, request) {
  ...
  if (!comment || !user) {
    response.status = 400;
  } else if (!comment.downvotedBy.includes(username)) {
    database.comments[id].downvotedBy.push(username);
    if (comment.upvotedBy.includes(username)) {
      const pos = comment.upvotedBy.indexOf(username);
      database.comments[id].upvotedBy.splice(pos,1);
    }
    response.status = 200;
    response.body = {};
    response.body.comment = database.comments[id];
...
// resource used: https://www.npmjs.com/package/read-yaml
function loadDatabase() {
  const readYaml = require('read-yaml');
  return readYaml.sync('database.yml');
}

// resource used: https://www.npmjs.com/package/write-yaml
